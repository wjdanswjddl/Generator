...
void MECGenerator::GenerateNSVInitialHadrons(GHepRecord * event) const
{
    ...
    while(!accept){
        ...
        // Make a temporary clone of the interaction that we can use for the calls
        // to genie::utils::BindHitNucleon() below. This is solely so that we can
        // use a configurable removal energy without duplicating code from QE
        Interaction temp_inter( *interaction );
        ...
        // Copy the local Target into the dummy interaction
        *temp_inter.InitState().TgtPtr() = tgt;
        // This call to BindHitNucleon retrieves the appropriate
        // removal energy
        utils::BindHitNucleon(temp_inter, *fNuclModel,
          removalenergy1, fHitNucleonBindingMode);

        tgt.SetHitNucPdg(pdgv[1]);
        fNuclModel->GenerateNucleon(tgt);
        p32i = fNuclModel->Momentum3();

        // Play the same removal energy game for the second nucleon
        *temp_inter.InitState().TgtPtr() = tgt;
        utils::BindHitNucleon(temp_inter, *fNuclModel,
          removalenergy2, fHitNucleonBindingMode);

        LOG("MECGenerator", pDEBUG) << "Eb1 = " << removalenergy1
          << " Eb2 = " << removalenergy2;
...
void MECGenerator::LoadConfig(void)
{
    ...
    // Decide how to handle the binding energy of the initial state nucleons.
    // This correction is only applied for the Valencia MEC model.
