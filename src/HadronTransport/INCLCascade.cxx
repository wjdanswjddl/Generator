...
void INCLCascade::ProcessEventRecord(GHepRecord * evrec) const{
  ...
  inputincl<<"number-shots = "<<1<<endl;
  inputincl<<"projectile = "<<pdglib->Find(evrec->Probe()->Pdg())->GetName()<<endl;
  inputincl<<"target = "<<pdglib->Find(evrec->TargetNucleus()->Pdg())->GetName()<<endl;
  inputincl<<"energy = "<<(evrec->Probe()->KinE())*1000<<endl;
 //ifstream inputincl;
 int arg =2;
 // Hafindra eto ilay any @ny gEvGenHandroNucleus d evrec no angalana azy
 //inputincl.open("inputincl.in");
...
if(!theINCLModel){
 ...
 // Loop over GHEP and run intranuclear rescattering on handled particles
  ...
  while( (p = (GHepParticle *) piter.Next()) )
  {
   ...
   int i =0;
   G4INCL::Random::saveSeeds();
  // std::cout<< " Random Seed   ********* "<< G4INCL::Random::getSeeds()<< std::endl;
   G4INCL::EventInfo result;
 ...
 if(Zexception <=0 || Zexception<=0){ // Make sure that ARemn and Zremn >0 
    sp->SetFirstMother(icurr); 
    sp->SetStatus(kIStStableFinalState);
    evrec->AddParticle(*sp);
    delete sp;
...
std::cout<<" Conservation of charge and Mass \n"<< " A_f= "<< A_f <<" A_i= "<< A_i <<" Z_f= "<<Z_f<<" Z_i= "<<Z_i<<std::endl;
} 
if(ListeOfINCLresult.size()==0){
