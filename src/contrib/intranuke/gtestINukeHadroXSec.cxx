...
INukeFateHA_t FindhAFate(const GHepRecord * evrec)
{
  ...
  int numtype[] = {kPdgProton, kPdgNeutron, kPdgPiP, kPdgPiM, kPdgPi0, kPdgKP, kPdgKM, kPdgK0, kPdgGamma,kPdgDeuteron,kPdgTritium,kPdgAlpha,kPdgHe3};
  ...
  while((p=(GHepParticle *) piter.Next()))
  {
    ...
    if(status==kIStStableFinalState)
    {
      switch((int) p->Pdg())
      {
        ...
        case ((int)kPdgAlpha)    : index =11; break;
        case ((int)kPdgHe3)      : index =12; break;
  ...
  if(num_nu==0){
  if (numtype[9]==kPdgDeuteron||numtype[10]==kPdgTritium||numtype[11]==kPdgAlpha||numtype[12]==kPdgHe3) {
         return kIHAFtKo;
        }
  }
      else if(num_nu==1)
      {
        ...
        else if (numtype[9]==kPdgDeuteron||numtype[10]==kPdgTritium||numtype[11]==kPdgAlpha||numtype[12]==kPdgHe3) {
         return kIHAFtKo;
